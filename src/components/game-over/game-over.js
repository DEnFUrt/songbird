...
import viktorySong from '../../media/victory.mp3';
  ...
  return (
    <div className={cl(s.gameOver_block, 'media', 'rounded-lg', 'bg-dark')}>
      <div className={cl(s.gameOver_block__body, 'media-body')}>
        ...
        {totalScore < 30 ? <BtnLevel {...{btnTitle}} /> : <Сongratulation {...{viktorySong}} /> }
      </div>
    </div>
  )
};
...
const Сongratulation = ({viktorySong}) => {
  return (
    <div className={cl('media', 'mt-3', 'd-flex')}>
      ...
      <audio src={viktorySong} autoPlay="autoplay" className={cl('d-none')}></audio>
    </div>
  )
}